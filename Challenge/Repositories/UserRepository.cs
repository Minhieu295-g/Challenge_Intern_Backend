...
namespace DotNetTraining.Repositories
{
    public class UserRepository(IDbConnection connection) : SimpleCrudRepository<User, Guid>(connection)
    {
        ...
        public async Task<String?> GetRoleUserByEmail(string email)
        {
            try
            {
                var sql = " SELECT r.Name FROM Users u JOIN UserRoles ur ON u.Id = ur.UserId JOIN Roles r ON ur.RoleId = r.Id WHERE u.Email = @Email";
        ...
        public async Task<User?> GetUserByEmailAndPassword(string email, string password)
        {
            ...
            {
                Console.WriteLine($"Error in GetUserByUsernameAndPassword: {e.Message} ");
                return null;
            }
        }
